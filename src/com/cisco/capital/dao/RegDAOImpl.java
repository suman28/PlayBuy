...
public class RegDAOImpl implements RegDAO {
	...
	public List<RegistrationVO> getRegisteredUsers() {
		...
		try {
			...
			while (rs.next()) {
				...
				/*registrationVO.setCountry(rs.getString(9));*/
				registrationList.add(registrationVO);
	...
	public void saveUser(RegistrationVO registrationVO) {
		...
		try {
			...
			/*pst.setString(9, registrationVO.getCountry());*/
			pst.executeUpdate();
	...
	public RegistrationVO getUserData(String userId) {
		...
		try {
			...
			while (rs.next()) {
				...
				registrationVO.setEmail(rs.getString(6));
				registrationVO.setContactNo(rs.getString(7));
				/*registrationVO.setCountry(rs.getString(8));*/
				registrationVO.setUserName(rs.getString(8));
	...
	public void editUser(RegistrationVO registrationVO) {
		...
		try {
			...
			/*pst.setString(8, registrationVO.getCountry());*/
			pst.setString(8, registrationVO.getUserId());

			pst.executeUpdate();

			/*
			 * while(pst.executeUpdate() > 0){ registrationVO = new
			 * RegistrationVO(); registrationVO.setFirstName("FIRST_NAME");
...
/*	public Map<String, String> getCountry(RegistrationVO registrationVO){
		...
		try {
			con = jdbcTemplate.getDataSource().getConnection();
			String countryQuery = "SELECT COUNTRY_CODE,COUNTRY_NAME FROM COUNTRY_TABLE where COUNTRY_CODE=?";
		
			pst = con.prepareStatement(countryQuery);
			pst.setString(1, registrationVO.getCountry_code());
			rs = pst.executeQuery();
			countryMap=new LinkedHashMap<String, String>();
			
			while (rs.next()) {
				countryMap.put(rs.getString("COUNTRY_CODE"), rs.getString("COUNTRY_NAME"));
				registrationVO = new RegistrationVO();
				registrationVO.setCountry_code(rs.getString("COUNTRY_CODE"));
				registrationVO.setCountry_name(rs.getString("COUNTRY_NAME"));
